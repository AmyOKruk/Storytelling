...
function ready(datapoints) {
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('cx', function (d) {
    ...
    return yPositionScale(d.Close);
  }).attr('r', 3).attr('fill', '#4cc1fc').on('mouseover', function (d) {
    // mouseover is the event for the cursor going over something.
    // When the mouse goes over, let's look at the datapoint
    console.log('My mouse clicked the datapoint', d); // if we want to talk about the actual circle, we use d3.select(this)

    d3.select(this).attr('r', '5'); // We can use the datapoint to update elements elsewhere on the page

    d3.select('.col-3').append('p').text('Date: ').style('font-weight', 'bold').append('tspan').text(d.Date).style('font-weight', 'normal') // d3.select('.col-3')
    .append('p').text('Close: ').style('font-weight', 'bold').append('tspan').text(d.Close).style('font-weight', 'normal');
