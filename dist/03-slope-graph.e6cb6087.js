...
function ready(datapoints) {
  ...
  svg.selectAll('circle').data(datapoints).enter().append('circle').attr('r', 3).attr('cx', function (d) {
    ...
    d3.selectAll('path' + '.' + this.getAttribute('class')).attr('stroke', 'red');
    d3.selectAll('circle' + '.' + this.getAttribute('class')).attr('fill', 'red');
    d3.selectAll('text' + '.' + this.getAttribute('class')).attr('fill', 'red');
  }).on('mouseout', function (d) {
    d3.selectAll('path' + '.' + this.getAttribute('class')).attr('stroke', function (d) {
      return colorScale(d.key);
    });
    d3.selectAll('circle' + '.' + this.getAttribute('class')).attr('fill', function (d) {
      return colorScale(d.state);
    });
    d3.selectAll('text' + '.' + this.getAttribute('class')).attr('fill', 'black');
  });
  ...
  svg.selectAll('text').data(nested).enter().append('text').attr('font-size', 12).attr('fill', '#333333').attr('x', xPositionScale('2016')).attr('dx', 5).attr('class', function (d) {
    return d.key.toLowerCase().replace(/[^a-z]*/g, '');
  }).attr('y', function (d) {
