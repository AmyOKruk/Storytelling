...
var height = 900 - margin.top - margin.bottom; // append the svg object to the body of the page

var svg = d3.select('#chart-10').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
d3.csv(require('/data/nodes5.csv')).then(ready).catch(function (err) {
...
function ready(data) {
  ...
  console.log(data); // Build X scales and axis:

  var x = d3.scaleBand().range([0, width]).domain(myGroups).padding(0.05);
  svg.append('g').call(d3.axisTop(x).tickSize(0)).selectAll('text').style('text-anchor', 'start').attr('transform', 'rotate(-45)').style('font-size', 8).attr('dy', -3).attr('id', function (d) {
  ...
  svg.append('g').call(d3.axisLeft(y).tickSize(0)).selectAll('text').style('font-size', 8).attr('class', function (d) {
    return d.toLowerCase().replace(/[^a-z]*/g, '') + ' ' + 'y_label';
  }).attr('dx', -3).select('.domain').remove(); // Build color scale

  var myColor = d3.scaleSequential(d3.interpolateBuPu).domain([1, 0]); // add the squares
  ...
  svg.selectAll().data(data, function (d) {
  ...
  }).style('stroke-width', 4).style('stroke', 'none').attr('class', function (d, i) {
    return d.variable.toLowerCase().replace(/[^a-z]*/g, '');
  }).attr('id', function (d, i) {
    return d.group.toLowerCase().replace(/[^a-z]*/g, '');
  }).on('mouseover', function (d) {
    d3.selectAll('text' + '.' + this.getAttribute('class')).style('font-size', 18);
    d3.selectAll('text' + '#' + this.getAttribute('id')).style('font-size', 18);
    var selectedClass = this.getAttribute('class');
    ...
    d3.select(this).style('stroke', 'black').style('stroke-width', 1).style('z-index', '9999');
  }).on('mouseout', function (d) {
    d3.selectAll('text' + '.' + this.getAttribute('class')).style('font-size', 8);
  ...
  svg.append('rect').attr('x', 416.06264869151465).attr('y', 58.06899286280727).attr('width', 187).attr('height', 206).attr('fill', 'none').attr('stroke', 'black').higher();
}
},{"d3":"../node_modules/d3/index.js","/data/nodes5.csv":"data/nodes5.csv"}]},{},["scripts/homework-13/10-test.js"], null)
//# sourceMappingURL=/10-test.8bdf9a9b.js.map